...
M.get_filename = function()
	...
	if not f.isempty(filename) then
		...
		vim.api.nvim_set_hl(0, "Winbar", { fg = "#6969DE", bold = true })
...
local get_gps = function()
	...
	if not require("core.functions").isempty(gps_location) then
		return require("plugins.configs.icons").ui.ChevronRight .. " " .. gps_location
	else
		return ""
	end
end
...
M.get_winbar = function()
	...
	if not f.isempty(value) and f.get_buf_option("mod") then
		local mod = "%#LspCodeLens#" .. require("plugins.configs.icons").ui.Circle .. "%*"
		if gps_added then
			value = value .. " " .. mod
		else
			value = value .. mod
